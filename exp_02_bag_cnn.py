...
from siamese_config import BAG_CONFIG
from siamese_utils import set_seed, get_device
from siamese_data import prepare_encoded_data, BagPairDataset
from siamese_models import SiameseBagPairClassifier
...
def plot_history(history):
    ...
    plt.xlabel("Epoch")
    plt.ylabel("Loss")
    plt.title("Bag CNN: Epoch vs Loss")
    ...
    plt.xlabel("Epoch")
    plt.ylabel("AUC")
    plt.title("Bag CNN: Epoch vs AUC")
    plt.legend(frameon=False)
    plt.tight_layout()
    plt.show()
...
def run_experiment(preprocess, X_train, X_val, X_test, T_train, T_val, T_test):
    ...
    train_ds = BagPairDataset(
        ...
        seed=cfg["seed"],
        within_bag_replace=cfg["within_bag_replace"],
        max_row_reuse=cfg["max_row_reuse"],
    )
    val_ds = BagPairDataset(
        Xva,
        Tva,
        bag_size=cfg["bag_size"],
    ...
    model = SiameseBagPairClassifier(
        input_dim=input_dim,
    ...
    print(f"\n=== TEST RESULTS (Bag CNN, bag_size = {cfg['bag_size']}) ===")
    ...
    plot_roc_curve(
        ...
        test_tpr,
        test_auc,
        title=f"ROC Curve (Test) — Bag CNN, bag_size={cfg['bag_size']}",
    )
